...
const buildHeroStats = (stats) => {
  const cards = stats
    .map((value) =>
      ...
      <div class="gh-stat-card">
        <span class="gh-stat-card__value">${value}</span>
      </div>
    `
        : ""
    )
  ...
  return cards ? `<div class="gh-hero__stat-grid">${cards}</div>` : "";
};

const wrapLayout = (blocks, modifier) => {
  ...
  return `
    <section class="gh-layout ${modifier}">
      ${items}
    </section>
  `;
};
...
`;
  ...
  const alumni = s.show_alumni_spotlight
    ? `
    <section class="gh-panel gh-panel--alumni">
      <header class="gh-panel__header">
        <div>
          <p class="gh-panel__eyebrow">Alumni Spotlight</p>
          <h2>Talk to alumni</h2>
          <p class="gh-panel__summary">Get real advice from people who have already made the leap.</p>
        </div>
      </header>
      <div class="gh-panel__body">
        <div class="gh-card-grid gh-card-grid--stretch gh-skeleton" data-block="alumni" data-endpoint="${escapeHtml(
          s.alumni_endpoint || ""
        )}">
  ...
  const mostTalked = s.show_most_talked
    ? `
    <section class="gh-panel gh-panel--topics">
      <header class="gh-panel__header">
        <div>
          <p class="gh-panel__eyebrow">Most Talked Topics</p>
          <h2>Trending conversations</h2>
        </div>
        <div class="gh-panel__toolbar">
          <label class="gh-select">
            <span>Timeframe</span>
      ...
      <div class="gh-panel__body">
        <div class="gh-card-grid gh-card-grid--topics gh-skeleton" data-block="most-talked" data-endpoint="${escapeHtml(
  ...
  const latest = s.show_latest
    ? `
    <section class="gh-panel gh-panel--latest">
      <header class="gh-panel__header">
        <div>
          <p class="gh-panel__eyebrow">Latest</p>
          <h2>Topics you may have missed</h2>
        </div>
        <a class="gh-link" href="/latest">View all</a>
      </header>
      <div class="gh-panel__body">
        <div class="gh-card-grid gh-card-grid--topics gh-skeleton" data-block="latest" data-endpoint="${escapeHtml(
  ...
  const courses = s.show_popular_courses
    ? `
    <section class="gh-panel gh-panel--courses">
      <header class="gh-panel__header">
        <div>
          <p class="gh-panel__eyebrow">Most Popular Courses</p>
          <h2>Courses loved by peers</h2>
          <p class="gh-panel__summary">Hand-picked programs and shortlists curated from community chatter.</p>
        </div>
      </header>
      <div class="gh-panel__body">
        <div class="gh-card-list gh-card-list--courses gh-skeleton" data-block="popular-courses" data-endpoint="${escapeHtml(
  ...
  const events = s.show_events
    ? `
    <section class="gh-panel gh-panel--events">
      <header class="gh-panel__header">
        <div>
          <p class="gh-panel__eyebrow">Upcoming Events</p>
          <h2>Join live workshops</h2>
          <p class="gh-panel__summary">Webinars, AMAs, and meetups designed to unblock your next milestone.</p>
        </div>
      </header>
      <div class="gh-panel__body">
  ...
  const categories = s.show_categories
    ? `
    <section class="gh-panel gh-panel--categories">
      <header class="gh-panel__header">
        <div>
          <p class="gh-panel__eyebrow">Explore Categories</p>
          <h2>Browse by interest</h2>
          <p class="gh-panel__summary">Jump into the spaces that match your current planning sprint.</p>
        </div>
        <a class="gh-link" href="/categories">View all</a>
      </header>
      <div class="gh-panel__body">
        <div class="gh-chip-cloud gh-skeleton" data-block="categories" data-endpoint="${escapeHtml(
    ...
    : "";
  const spotlightRow = wrapLayout([alumni, mostTalked], "gh-layout--spotlight");
  const insightsRow = wrapLayout([latest, courses], "gh-layout--insights");
  const exploreRow = wrapLayout([events, categories], "gh-layout--explore");

  return `${hero}${spotlightRow}${insightsRow}${exploreRow}`;
};
